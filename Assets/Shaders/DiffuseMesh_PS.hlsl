...
float4 main(VertexOutput_P4N4T4L4 input) : SV_TARGET
{
    ...
    float shadow = CalculateShadow(tShadowMap, tShadowMapSamplerState, input.lightPosition);

    float3 diffuseLight = max(dot(input.worldNormal.xyz, -directionLight.direction.xyz), 0.0f) * directionLight.color.xyz * directionLight.intensity.x;
