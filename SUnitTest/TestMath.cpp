...
public:
    ...
    TEST_METHOD(TestVectorNormalize) {
        Vector3f v1(1, 1, 1);
        Vector3f v2 = Normalize(v1);
        v1.Normalize();
        _ASSERTE(v2 == v1);
        _ASSERTE(comparison_traits<float>::equal(1, v2.Dot(v2)));
        _ASSERTE(comparison_traits<float>::equal(1, v1.Dot(v2)));
    }

    TEST_METHOD(TestVectorDotProduct) {
        Vector3f v1(2, 2, 2);
        Vector3f v2(3, 3, 3);
        _ASSERTE(comparison_traits<float>::equal(18, v2.Dot(v1)));
    }
